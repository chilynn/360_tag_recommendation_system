...
def rankTopCoverage(top_coverage_category_info_dict,category_stat_dict,all_app_counter):
	...
	for category in category_stat_dict.keys():
		...
		pure_coverage_ratio = 1.0*(cover_app_counter)/all_app_counter
...
#获取类目候选词
def getCandidateCategory(category_path,filter_category_set,category_parent_dict,category_child_dict):
	#该字典结构key->候选类目,value->数组
	#数组第一个元素是以这个候选类目为根节点的子树上所有节点构成的集合，数组第二个元素类目集合覆盖的appid集合
	category_stat_dict = {}
	candidate_category_set = set([])
	node_children_dict = getNodeChildren(category_child_dict)
	...
	for category in candidate_category_set:
		...
		if category not in category_parent_dict.keys():
			...
			# if category in node_children_dict.keys():
			# 	category_stat_dict.setdefault(category,[node_children_dict[category],set()])
			# else:
			# 	category_stat_dict.setdefault(category,[set([category]),set()])
...
#从原来的维护父类关系转成维护子类关系
def converToCategoryChildDict(category_parent_dict):
	category_child_dict = {}
	for category in category_parent_dict.keys():
		category_child_dict.setdefault(category,set([]))
		for parent in category_parent_dict[category]:
			parent_name = parent[0]
			relation = parent[1]
			category_child_dict.setdefault(parent_name,set([])).add((category,relation))
	return category_child_dict

#获取根节点集合，一个category节点可以属于多个root节点的category
def getRootSet(category_parent_dict,root_set,to_handle_set):
	if len(to_handle_set) == 0:
		return root_set
	for parent in to_handle_set:
		parent_name = parent[0]
		relation = parent[1]
		#父类为空，则认为它是根节点,添加到最终要输出的root_set中
		if len(category_parent_dict[parent_name]) == 0:
			root_set.add(parent_name)
		#如果它不是根节点，则把它所有的父节点加入到待处理的to_handle_set中，直到to_handle_set为空才结束
		else:
			to_handle_set = to_handle_set | category_parent_dict[parent_name]
		#从to_handle_set中去掉已经处理过的节点
		to_handle_set = to_handle_set - set([parent])
	return getRootSet(category_parent_dict,root_set,to_handle_set)
...
#获取每一个节点下面的所有孩子
def getChildSet(category_child_dict,child_set,to_handle_set):
	if len(to_handle_set) == 0:
		return child_set
	for child in to_handle_set:
		child_name = child[0]
		relation = child[1]
		child_set.add(child_name)
		to_handle_set = to_handle_set | category_child_dict[child_name]
		to_handle_set = to_handle_set - set([child])
	return getChildSet(category_child_dict,child_set,to_handle_set)

#获取每一个节点下面的所有孩子
def getNodeChildren(category_child_dict):
	print '-aggregate category with same node'
	node_children_dict = {}
	for category in category_child_dict.keys():
		child_set =  getChildSet(category_child_dict,set([]),category_child_dict[category])
		for child in child_set:
			# if category == u'保险':
			# 	print child
			node_children_dict.setdefault(category,set([])).add(child)
...
def main(category_path):
	...
	category_child_dict = converToCategoryChildDict(category_parent_dict)

	#候选类目词的关系树
	category_stat_dict = getCandidateCategory(category_path,filter_category_set,category_parent_dict,category_child_dict)
